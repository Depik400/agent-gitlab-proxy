...
4. Check GitLab proxy configuration:
   - Run `gitlab-proxy config`.
   - If `default_host` is configured, omit `--host-name` in later commands unless the user requested a different host.
   - If no default host is configured and the remote host matches exactly one configured host URL, pass that host with `--host-name`.
...
6. Fetch review context:
   - Run `gitlab-proxy mr-context --repo <repo> --branch <BranchName>` when a default host exists.
   - Otherwise add `--host-name <host>` to the command.
